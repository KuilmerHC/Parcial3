...
int main () {
    ...
    // ----------------------------------- Definir ataques ---------------------------------------------
    Ataque placaje("Placaje", 5);
    Ataque latigoCepa("LatigoCepa",10);
    Ataque ascuas("Ascuas", 5);

    Pokemon jugador ("Frisby Colombiano", Tipo::PLANTA, 1, 45, 20, 49);
    jugador.aprenderAtaque(placaje);
    jugador.aprenderAtaque(latigoCepa);

    Pokemon oponente ("Frisby Copia", Tipo::FUEGO, 1, 39, 20, 42);
    oponente.aprenderAtaque(Ataque("aranazo", 10));
    oponente.aprenderAtaque(ascuas);
                ...
                if (turnoActual == EstadoTurno::JUGADOR_ELIGE_ATAQUE && jugador.estaVivo() && oponente.estaVivo()) {
                    ...
                    if (ataqueSeleccionadoIndex != -1 && ataqueSeleccionadoIndex < static_cast<int>(jugador.getAtaques().size())) {
                        ...
                        if (!oponente.estaVivo()) {
                            std::cout << oponente.getNombre() << " ha sido derrotado!" << std::endl;
                            mensajeCombateText.setString(oponente.getNombre() + " ha sido derrotado!");
                ...
                if (oponente.estaVivo() && jugador.estaVivo()) {
                    if (!oponente.getAtaques().empty()) {
                        ...
                        if (!jugador.estaVivo()) {
                            ...
                            mensajeCombateText.setString(jugador.getNombre() + " ha sido derrotado!");
                            turnoActual = EstadoTurno::COMBATE_TERMINADO;
                            esperandoFinDeAccion = false;
                        } else {
                            esperandoFinDeAccion = true;
